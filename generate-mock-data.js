...
function main() {
  const blogsFolder = path.join(__dirname, 'public', 'blogs');
  const blogsData = [];
  ...
  if (fs.existsSync(blogsFolder)) {
    ...
    for (const file of files) {
      ...
      contentHtml = contentHtml.replace(/<\/div>\s*<\/body>\s*<\/html>/gi, '').trim();
      ...
      blogsData.push({
        ...
        datePublished: metaInfo.datePublished,
        dateModified: metaInfo.datePublished,
        seo: {
  ...
  // Construct ts file content
  const tsContent = `// Fallback static mock data for local run and CI/CD builds
...
export const testimonials: IMockTestimonial[] = ${JSON.stringify(testimonialsData, null, 2)};

export const services: IMockService[] = ${JSON.stringify(servicesData, null, 2)};

export const faqs: IMockFAQ[] = ${JSON.stringify(faqsData, null, 2)};

export const pages: Record<string, IMockPage> = ${JSON.stringify(pagesData, null, 2)};

export const blogs: IMockBlog[] = ${JSON.stringify(blogsData, null, 2)};
`;

  fs.writeFileSync(path.join(__dirname, 'src', 'lib', 'mockData.ts'), tsContent, 'utf8');
  console.log('Successfully generated src/lib/mockData.ts containing all dynamic parsed values!');
}
